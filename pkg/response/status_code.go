...
// statusCodeTextMap 用于存储状态码Key映射关系
var statusCodeTextMap = struct {
	sync.RWMutex
...
func SetStatusCode(code StatusCode, msg string) {
	statusCodeTextMap.Lock()
	defer statusCodeTextMap.Unlock()
	statusCodeTextMap.mapping[code] = msg
}

// GetStatusCodeText 根据状态码获取对应的错误消息
func GetStatusCodeText(code StatusCode) string {
	statusCodeTextMap.RLock()
	defer statusCodeTextMap.RUnlock()
	return statusCodeTextMap.mapping[code]
}
